...
using ControlCenter.Shared.Extensions;
...
public abstract class BaseApiClientProvider : IApiClientProvider
{
    ...
    public virtual IApiClientProvider UseAuthenticationBearer(string bearerToken = null)
    {
    ...
    public virtual IApiClientProvider UseBaseUrl(string baseUrl)
    {
        _apiUrlBase = baseUrl;
    ...
    protected void AddAuthenticationHeader(
        string authenticationType, string token)
    {
    ...
    protected async Task<ApiClientResult<T>> ConvertResponseToApiClientResultAsync<T>(
        HttpResponseMessage response = null, Exception ex = null)
    {
        string content = null;
    ...
    public abstract Task<ApiClientResult<T>> PostAsync<T>(string apiRoute, object body) where T : new();
}
