...
public sealed class HttpClientApiClientProvider : BaseApiClientProvider
{
    ...
    public override async Task<ApiClientResult<T>> GetAsync<T>(string apiRoute)
    {
        _apiRoute = apiRoute;
    ...
    public override async Task<ApiClientResult<T>> PostAsync<T>(string apiRoute, object body)
    {
        _apiRoute = apiRoute;
        ...
        try
        {
            ...
            var response = await _httpClient.PostAsync($"{_apiUrlBase}{apiRoute}",
                new StringContent(body.ToJson(), Encoding.UTF8, _contentType));
